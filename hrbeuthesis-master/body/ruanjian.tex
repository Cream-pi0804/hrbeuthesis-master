...
图\ref{fig:hardware_topo}展示了试验样机硬件系统的总体拓扑结构。
...
\begin{itemize}
    \item \textbf{动力供电链路}：24\,V 总电源直接馈入 3 个电机驱动器。
    这种高压直供模式保证了伺服电机在瞬态加速和重载工况下具备充足的功率冗余。
    \item \textbf{控制供电链路}：为了满足低压电路的需求，
...
\begin{itemize}
    \item \textbf{主从通信总线}：树莓派作为舱内的上位运算节点，
    通过 USB 接口与下位机 STM32 建立串口通信，
    负责下发规划轨迹并接收底层实时状态。
    \item \textbf{执行器控制总线}：STM32 扩展板作为底层控制中枢，
...
\begin{itemize}
    \item \textbf{视觉感知接口}：单目相机通过网线直连至树莓派的千兆网口。为后续的 ROS 2 视觉目标解算提供数据源。
    \item \textbf{外部监控链路}：树莓派通过另一路网线，经由防水接插件穿透水下密封舱壁，连接至控制系统。该线路承载视频流和 ROS 2 话题控制指令，使操作人员能够进行实时监测与控制。
\end{itemize}
...
\begin{itemize}
    \item \textbf{数据对齐}：针对 16 位控制字，系统采用
     ...
     线传输中的一致性。
    \item \textbf{方向自适应映射}：根据设定角动反转报文的方向位
    掩码。此举简化了电机旋转方向的修改。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{执行抓取}：机械臂到达预定点后，指令执行下降并触发手爪闭合，同时监测确认抓取成功。
...
\begin{itemize}
    ...
    \item \textbf{运动学与轨迹规划验证}：利用逆运动学算法，给定末端目标位姿，
    测试机械臂在笛卡尔空间下的轨迹规划能力。
    \item \textbf{视觉引导下的闭环抓取实验}：集成 OpenCV 与 ROS2 环境下的视觉定位节点，
...
\begin{itemize}
    ...
    \item \textbf{绳索传动特性}：绳驱系统存在的弹性形变与摩擦，在一定程度上影响了末端轨迹的跟随精度。
    \item \textbf{视觉反馈频率}：摄像机分辨率及图像处理抓取过程中的补偿精度产生了一定误差。
\end{itemize}
尽管存在一些问题，但整体表现满足设计预期，验证了设计方案的可行性。
\BiSection{本章小结}{Summary}

围绕水下绳驱机械臂的物理试制需求，本章完成了底层控制板与上层分布式软件的闭环搭建，并对整机开展了实操性能评估。
\begin{enumerate}
    \item \textbf{软硬件协同控制体系构建}：搭建了以 STM32 微控制器为核心的底层驱动系统，通过设计基于 DMA 循环缓冲与串口空闲中断的通信机制，并引入 CRC-8 数据校验算法，有效规避了多轴伺服指令在传输过程中的丢包与堵塞隐患，保障了底层数据交互的稳定性。
    \item \textbf{基于 ROS 2 的分布式软件架构设计}：利用 ROS 2 的模块化解耦合特性，构建了涵盖感知、调度、规划与执行四层逻辑的控制软件平台。实现了基于 ROI 交互的视觉定位模型、标准 G 代码轨迹插补算法以及任务调度状态机，打通了从视觉识别到动作执行的完整数据链路，实现了对机械臂作业流程的自动化管理。
    \item \textbf{系统运动性能与视觉抓取验证}：在实验室环境下开展了样机试验。实验结果表明，机械臂能够精准执行复位、空间直线与圆弧轨迹规划等预定任务。视觉引导下的目标抓取动作连贯，系统反馈链路响应及时，证明了软硬件联调方案切实可行。
    \item \textbf{误差来源深度剖析}：通过对比实验数据，分析了装配公差、绳索弹性形变及图像处理延迟对末端定位精度的影响。尽管存在客观的物理误差，但系统整体运行稳定，功能表现符合开发初衷。
\end{enumerate}

本章的联调测试将前文的运动学模型与轨迹规划算法成功落地于物理实体。这排除了前期理论推导的盲区，为该型设备未来在复杂水下环境中的进阶应用扫清了工程障碍。